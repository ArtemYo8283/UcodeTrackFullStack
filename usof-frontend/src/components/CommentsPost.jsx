...
export default function CommentsPost ({ idComment, comment, users }) {
	...
	useEffect(() => {
		...
		isUserLike.find((item) => item.author_id === currentUser);
		...
		<div className="flex-col w-full py-4 mx-auto mt-3 bg-white border-b-2 border-r-2 border-gray-200 sm:px-4 sm:py-4 md:px-4 sm:rounded-lg sm:shadow-sm md:w-2/3">
			<div className="flex flex-row md-10">
					...
					{isEdit ? (
						<div className="flex-1 px-2 ml-2 text-sm font-medium leading-loose text-gray-600">
							...
							>
								...
								<button className="px-3 text-white border border-orange-300 bg-orange-400 mt-1 rounded ease-in duration-300 hover:border-orange-600  hover:text-orange-400">
									Edit
								</button>
								<label
									className="px-3 text-white border border-orange-300 bg-orange-400 mt-1 cursor-pointer rounded ease-in duration-300 hover:border-orange-600 hover:text-orange-400"
				...
				<div className="flex items-end">
					{!isEdit ? (
					<button
						className="px-3 text-white bg-orange-400 mt-1 border bg-orange-500 text-white font-bold rounded ease-in duration-300 hover:border-orange-600 hover:bg-white hover:text-orange-400"
